...
$date = validate($jsonData->date);
  ...
  // create db query
  $query = $writeDB->prepare('INSERT into supply (suppliers_id, particulars, quantity, debit, credit, date) values (:supplier_id, :particulars, :quantity, :debit, :credit, :date)');
  ...
  $response->addMessage("Supply Successfully Added");
  $response->send();
  exit;
...
// catch error
  ...
  $response->setHttpStatusCode(500);
  $response->setSuccess(false);
  $response->addMessage("There was an error while creating supply - please try again");
  $response->send();
  exit;
}
