...
if(!isset($_SERVER['HTTP_AUTHORIZATION']) || strlen($_SERVER['HTTP_AUTHORIZATION']) < 1) {
  ...
  sleep(1);
    ...
    try {
      ...
      // create db query
$query = $readDB->prepare('SELECT id, name,beat from suppliers WHERE deleted="N"');
      $query->execute();
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)) {
       //   get total amount of credit and debit per supplier
       $querys = $readDB->prepare('SELECT SUM(debit),SUM(credit)  from supply WHERE deleted="N" AND suppliers_id =:id');
       $querys->bindParam(':id', $row['id'], PDO::PARAM_STR);
       $querys->execute();
       $total = $querys->fetch(PDO::FETCH_NUM);
       $summ = $total[1]-$total[0]; 

        $task = array("id"=> $row['id'],"name"=> $row['name'],"beat"=> $row['beat'],"amount" => $summ ? $summ :0 );
        // create task and store in array for return in json data
        $taskArray[] =  $task;
      }
      
    //   get total amount of credit and debit for all suppliers
    ...
    //    return $sumtotal; 
      ...
      $returnData['suppliers'] = $taskArray;
      ...
      $response->addMessage("All Suppliers Fetched");
      $response->send();
      exit;
    }
    // if error with sql query return a json error
      ...
      $response->setHttpStatusCode(500);
      $response->setSuccess(false);
      $response->addMessage("Failed to get suplliers");
      $response->send();
