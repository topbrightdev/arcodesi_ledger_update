...
if (!isset($jsonData->supplierName) ) {
  $response = new Response();
  $response->setHttpStatusCode(400);
  $response->setSuccess(false);
  (!isset($jsonData->supplierName) ? $response->addMessage("supplier Name not set") : false);
  $response->send();
  exit;
}

// check to make sure that supplier Id are not empty and not greater than 255 characters
if (strlen($jsonData->supplierName) < 1 || strlen($jsonData->supplierName) > 255 ) {
  $response = new Response();
  $response->setHttpStatusCode(400);
  $response->setSuccess(false);
  (strlen($jsonData->supplierName) < 1 ? $response->addMessage("supplier Name cannot be blank") : false);
  (strlen($jsonData->supplierName) > 255 ? $response->addMessage("supplier Name must be less than 255 characters") : false);
  $response->send();
  exit;
}

// attempt to query the database to delete supplier - use write connection as it needs to be synchronous for oldsupplier/token
try {
  $supplierName = validate($jsonData->supplierName);
  
//   $supplierName = $jsonData->supplierName;
          // create db query
$query = $writeDB->prepare('SELECT id from suppliers WHERE deleted="Y" AND name =:name');
  ...
  exit;
    ...
    // get returned row
  $rowsup = $query->fetch(PDO::FETCH_ASSOC);
  
   $supplierId= $rowsup['id'];

$query = $writeDB->prepare('UPDATE supply SET deleted="N" where suppliers_id = :supplierid');
      
      $query->bindParam(':supplierid', $supplierId, PDO::PARAM_STR);
      $query->execute();

  $query = $writeDB->prepare('UPDATE suppliers SET deleted="N" where id = :supplierid');
      ...
      if($rowCount === 0) {
        ...
        $response->setHttpStatusCode(400);
        $response->setSuccess(false);
        $response->addMessage("Failed to Restore supplier");
        $response->send();
        exit;
      }
  ...
  $response->addMessage("Supplier Successfully Restored");
  $response->send();
  exit;
...
// catch error
  ...
  $response->setHttpStatusCode(500);
  $response->setSuccess(false);
  $response->addMessage("There was an  error  while Restoring supplier - please try again");
  $response->send();
  exit;
}
