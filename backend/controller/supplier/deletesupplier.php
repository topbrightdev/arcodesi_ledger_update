...
if (strlen($jsonData->supplierId) < 1 || strlen($jsonData->supplierId) > 255 ) {
  $response = new Response();
  $response->setHttpStatusCode(400);
  $response->setSuccess(false);
...
$query = $writeDB->prepare('UPDATE supply SET deleted="Y" where suppliers_id = :supplierid');
      
      $query->bindParam(':supplierid', $supplierId, PDO::PARAM_STR);
      $query->execute();

  $query = $writeDB->prepare('UPDATE suppliers SET deleted="Y" where id = :supplierid');
      ...
      if($rowCount === 0) {
        ...
        $response->setHttpStatusCode(400);
        $response->setSuccess(false);
        $response->addMessage("Failed to delete supplier");
        $response->send();
        exit;
      }
  ...
  $response->addMessage("Supplier Successfully Deleted");
  $response->send();
  exit;
...
// catch error
  ...
  $response->setHttpStatusCode(500);
  $response->setSuccess(false);
  $response->addMessage("There was an  error  while deleting supplier - please try again");
  $response->send();
  exit;
}
