...
$db = mysqli_connect('localhost', 'root', '', 'arcodesi_ledger');
...
if (isset($_POST['login_admin'])) {
  ...
  // $2y$10$/eGJ8h06yYZYli3RuKpZRe2gF5DUVsu5URaj6cHkGWoJQXZ9.xnz6

  if (count($errors) == 0) {
    $password = md5($password);
    $query = "SELECT * FROM tblusers WHERE email = '$admin_username' AND password ='$password'";
    $full_name = "SELECT * FROM tblusers WHERE email = '$admin_username'";
    $results = mysqli_query($db, $query);
    $results2 = mysqli_query($db, $full_name);
